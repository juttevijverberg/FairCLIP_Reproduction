...
#Additional PD metrics to add
def compute_group_precision(preds, gts, attrs, group):
    """
    ...
    """
    group_indices = (attrs == group)
    preds_group = preds[group_indices]
    gts_group = gts[group_indices]

    if len(preds_group) == 0:  # Handle edge case where no samples belong to the group
        return 0.0

    precision = precision_score(gts_group, preds_group, zero_division=0)
    return precision


def compute_precision_disparity(preds, gts, attrs):
    """
    ...
    for group in unique_groups:
        ...
        # Handle edge case where no samples belong to the group
        if len(preds_group) == 0:
            precision = 0.0
        else:
            precision = precision_score(
                gts_group, preds_group, zero_division=0)

        group_precisions.append(precision)

    # Compute Precision Disparity (absolute difference in precision)
    pd = abs(max(group_precisions) - min(group_precisions))
...
def evalute_perf_by_attr(preds, gts, attrs=None, num_classes=2):
    ...
    eods = []
    eors = []
    pdds = []  # Add Precision Disparity metrics
    ...
    for i in range(attrs.shape[0]):
        ...
        if num_classes == 2:
            ...
            eor = equalized_odds_ratio(gts,
                                        pred_labels,
                                        sensitive_features=attr)
            pdd = compute_precision_disparity(gts,
                                        pred_labels, attr)
        elif num_classes > 2:
            ...
            eor = 0
            pdd = compute_precision_disparity(gts, pred_labels, attr)

        dpds.append(dpd)
        eods.append(eod)
        pdds.append(pdd)

    return esaccs_by_attrs, esaucs_by_attrs, aucs_by_attrs, dpds, eods, pdds
...
def evalute_comprehensive_perf(preds, gts, attrs=None, num_classes=2):
    ...
    return overall_acc, esaccs_by_attrs, overall_auc, esaucs_by_attrs, aucs_by_attrs, dpds, eods, between_group_disparity, pdds
